...
class GenerateFeaturesTask:
    ...
    def run_task(self) -> None:
        ...
        df, new_cols = transform_continuous_covariates(df, variables=self.density_vars)
...
def create_dummy_columns(
    ...
    df_dummies = df_dummies.rename({old: new for old, new in zip(old_cols, new_cols)})
...
@staticmethod
def create_land_use_dummies(df: pl.DataFrame, col_order: list[str]) -> pl.DataFrame:
    """
    ...
    df_dummies_lu = create_dummy_columns(
        df=df,
...
def combine_land_use_intensity_columns(
    ...
    """
    Creates a combined land-use and land-use intensity column and generate
    dummy columns based on this.

    Args:
        - df: DataFrame containing 'Predominant_land_use' and 'Use_intensity'.
        - col_order: Order of the combined dummy variables in the output df.
    ...
    df_dummies_comb = create_dummy_columns(
        df=df,
...
def group_land_use_types_and_intensities(
    ...
    """
    Creates certain special groupings of land-use types and intensities. This
    is based on the latest published version of the BII model, see De Palma
    et al (2019). This is mainly motivated by limited data in these land use
    types. It includes the following groupings:
        - All secondary vegetation types are combined into one column, but with
    ...
    def _secondary_veg_intensity(row: pl.String) -> str:
        if "secondary" in str(row).lower():
            new_row = "Secondary vegetation_" + str(row).split("_")[1]  # keep intensity
            return new_row
        else:
            return row
    ...
    df = df.with_columns(
        ...
        .then(
            ...
            .otherwise(pl.lit("Secondary vegetation_Intense use"))  # Other --> Intense
        )
    ...
    df_dummies_secondary_veg = create_dummy_columns(
        df=df,
...
def transform_continuous_covariates(
    ...
    and adds the transformed columns to the dataframe. The transformations
    include log, square root and cube root.
    ...
    for col in variables:
        ...
        df = df.with_columns(pl.col(col).pow(1 / 3).alias(f"{col}_cbrt"))  # Cube root
...
def calculate_study_mean_densities(
    ...
    """
    Calculate the within-study mean values for the selected variables, for each
    level of resolution that those covariates exist. Currently, population and
    road density columns are included. These can act as control variables on
    sampling bias due to higher accessibility.

    NOTE: Doing the same for bioclimatic and topographic variables should be
    considered, as they can act as control variables for environmental factors
    that might otherwise bias the inference.

    Args:
        - df: Dataframe containing all columns in the 'variables' list.
        - variables: Columns for which within-study means should be calculated.
