...
class AlphaDiversityFeatureTask:
    """
    Task to create dataframes for modeling of alpha diversity. The alpha
    metrics currently included are total species abundance, species richness
    and Shannon index (all at the site level). Each dataframe aggregates
    abundance data at different levels of the taxonomic hierarchy (Kingdom,
    Phylum, Class, Order). This pre-computation allows for experimenting with
    different levels of hierarchical granularity in the model.
    """
    ...
    def run_task(self) -> None:
        ...
        # hierarchy, another column is added to the base list (Kingdom,
        # Phylum, ...)
        groupby_cols = ["SS", "SSB", "SSBS"]

        for i, path in enumerate(self.abundance_data):
            df_total_abund = calculate_total_abundance(df, groupby_cols=groupby_cols)
            df_mean_abund = calculate_mean_abundance(df, groupby_cols=groupby_cols)
            df_richness = calculate_species_richness(df, groupby_cols=groupby_cols)
            df_shannon = calculate_shannon_index(df, groupby_cols=groupby_cols)
            ...
            # that contains four levels of the taxonomic hierarchy. We continue
